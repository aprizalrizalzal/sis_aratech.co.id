...
<template>
    <div class="grid grid-cols-1 mt-4 gap-4">
        <div v-for="header in $page.props.headers" :key="header.id"
            ...
            <h1 class="text-xl font-semibold text-green-800 mt-2">
                SIService - {{ header.company }}
            </h1>
            <p class="text-sm/relaxed text-gray-400">
            ...
            <p class="text-sm/relaxed text-gray-400">Pastikan Service Code yang dimasukkan menggunakan huruf kapital
                agar sistem dapat
                menemukan data yang sesuai dengan benar. Contoh: <strong>DFGYHN</strong>.</p>
            <p class="text-sm/relaxed text-gray-400">Harap dicatat! Service Code hanya boleh menggunakan huruf kapital.
            </p>
            <div v-for="header in $page.props.headers" :key="header.id">
                <p class="mt-4 text-sm/relaxed text-gray-400">Untuk mengeksplorasi berbagai produk kami,
                    kunjungi situs
                    web kami <a target="_blank" rel="noopener noreferrer" class="text-green-600 font-bold"
    ...
    <Modal v-if="service" :show="showModal">
        <div class="p-6">
            ...
            <table class="table-auto w-full mt-4">
                <tbody>
                    <tr class="font-bold border border-green-200 bg-green-50 text-green-800 py-2 px-4">
                        <td class="text-green-800">Service Code</td>
                        <td> {{ service.service_code }}</td>
                    </tr>
                    <tr class="py-2 px-4">
                        <td class="text-green-800">Email</td>
                        <td>{{ service.customer.user.email }}</td>
                    </tr>
                    <tr class="py-2 px-4">
                        <td class="text-green-800">Phone</td>
                    ...
                    <tr class="py-2 px-4">
                        <td class="text-green-800">Device Type</td>
                        <td>{{ service.device.device_type.type_name }}</td>
                    </tr>
                    <tr class="py-2 px-4">
                        <td class="text-green-800">Model</td>
                        <td>{{ service.device.model }}</td>
                    </tr>
                    <tr class="py-2 px-4">
                        <td class="text-green-800">Serial Number</td>
                        <td>{{ service.device.serial_number }}</td>
                    </tr>
                    <tr class="py-2 px-4">
                        <td class="text-green-800">Warranty Status </td>
                        <td>{{ service.status_warranty_service.status }} </td>
                    </tr>
                    <tr class="py-2 px-4">
                        <td class="text-green-800">Date Received</td>
                        <td>{{ service.date_received }}</td>
                    </tr>
                    <tr class="py-2 px-4">
                        <td class="text-green-800">Estimated Completion</td>
            ...
            <hr>
            <div class="mt-3">
                <p class="text-sm/relaxed text-gray-400">Jangan lupa untuk sering cek status servis Anda agar selalu
                    mendapatkan
    ...
    <Modal v-if="message" :show="showModal">
        <div class="p-6">
            ...
            <hr class="mt-4 mb-2">
            <P class="font-bold text-red-800 my-4">{{ message }}</P>
            <p class="text-sm/relaxed text-gray-400">Pastikan Service Code yang dimasukkan menggunakan huruf kapital
                agar sistem dapat
                menemukan data yang sesuai dengan benar. Contoh: <strong>DFGYHN</strong>.</p>
            <p class="text-sm/relaxed text-gray-400">Harap dicatat! Service Code hanya boleh menggunakan huruf kapital.
